Rad-Mem Landing Page Refinements
 ...
 Location: ~/Scripts/claude-mem-pro/
 Dev server: PM2 process claude-mem-pro-dev (restart with pm2 restart claude-mem-pro-dev)

 What Was Built This Session

 New landing page components created in src/components/landing/:
 - Hero.tsx - Updated with scroll-based fade (needs video transform fix)
 - AgentExplainer.tsx - "One agent takes notes about another" section
 - ContextTimeline.tsx - Mock timeline UI showing Before/Current/Next
 - WhyItWorks.tsx - Problem/Solution explanation
 - Differentiators.tsx - 6 novel features grid
 - ProtocolCTA.tsx - Final CTA "Real-Time Agent Data"

 Current Page Flow (in src/app/page.tsx)

 Hero → AgentExplainer → ContextTimeline → WhyItWorks → Differentiators → ProtocolCTA → Footer

 Vision

 Rad-Mem = "One agent taking notes about what another agent does"
 - Background video shows robot taking notes while watching another robot work
 - Video should transform from fullscreen background → framed inline as user scrolls
 - Before/Current/Next temporal awareness is the key differentiator
 ...
 Approach: Keep a single <video> element that starts fixed fullscreen and transforms via CSS/framer-motion into a framed inline element that "docks" into the
 AgentExplainer section.
