...
Rad-Mem uses a skill-based search architecture that provides intelligent memory retrieval through natural language queries. This replaced the MCP-based approach in v5.4.0, saving ~2,250 tokens per session start. The skill was enhanced and renamed to "mem-search" in v5.5.0 for better scope differentiation.
...
```bash
curl "http://localhost:38888/api/search/observations?query=bugs&type=bugfix&limit=5"
```
...
```markdown
# Rad-Mem mem-search Skill

Access rad-mem's persistent memory through a comprehensive HTTP API.
...
**Example**:
```bash
curl "http://localhost:38888/api/search/observations?query=authentication&type=decision&limit=5"
```
...
**Example**:
```bash
curl "http://localhost:38888/api/search/by-file?filePath=worker-service.ts&limit=10"
```
...
**Example**:
```bash
curl "http://localhost:38888/api/context/recent?project=rad-mem&limit=5"
```
...
# Search test
curl "http://localhost:38888/api/search/observations?query=test&limit=1"
```
