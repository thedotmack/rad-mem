---
title: "Configuration"
description: "Environment variables and settings for Rad-Mem"
---
...
| `CLAUDE_MEM_DATA_DIR`         | `~/.rad-mem/`                | Data directory (production default)   |
...
- **Production (installed plugin)**: `~/.rad-mem/` (always, regardless of CLAUDE_PLUGIN_ROOT)
- **Development**: Can be overridden with `CLAUDE_MEM_DATA_DIR`

```
~/.rad-mem/
├── rad-mem.db           # SQLite database
...
```json
{
  "description": "Rad-mem memory system hooks",
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
...
**Migration Note**: As of v5.4.0, Rad-Mem uses skill-based search instead of MCP tools. As of v5.5.0, the skill was renamed to "mem-search" for better scope differentiation.
...
Rad-Mem supports switching between stable and beta versions via the web viewer UI.

### Accessing Version Channel

1. Open the viewer at http://localhost:38888
