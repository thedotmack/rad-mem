# Progressive Disclosure: Rad-Mem's Context Priming Philosophy
...
## Rad-Mem's Solution: Progressive Disclosure
...
```markdown
*Use rad-mem MCP search to access records with the given ID*
```
...
Rad-Mem implements progressive disclosure in search results too:
...
- [Rad-Mem Architecture](architecture/overview) - How it all fits together
...
*This philosophy emerged from real-world usage of Rad-Mem across hundreds of coding sessions. The pattern works because it aligns with both human cognition and LLM attention mechanics.*
