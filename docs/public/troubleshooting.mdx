...
description: "Common issues and solutions for Rad-Mem"
---
...
**Symptoms**: Cannot access http://localhost:38888, page doesn't load, or shows connection error.
...
1. Check browser localStorage is enabled:
   ```javascript
   // In browser console
   localStorage.getItem('rad-mem-settings')
   ```

2. Verify settings endpoint is working:
   ```bash
   curl http://localhost:38888/api/settings
   ```

3. Clear localStorage and try again:
   ```javascript
   // In browser console
   localStorage.removeItem('rad-mem-settings')
   ```
...
1. Check SSE endpoint is accessible:
   ```bash
   curl -N http://localhost:38888/stream
   ```
...
1. Check if summaries exist:
   ```bash
   sqlite3 ~/.rad-mem/rad-mem.db "SELECT COUNT(*) FROM session_summaries;"
   ```
...
1. Check FTS5 tables exist:
   ```bash
   sqlite3 ~/.rad-mem/rad-mem.db "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts';"
   ```
...
3. Check triggers exist:
   ```bash
   sqlite3 ~/.rad-mem/rad-mem.db "SELECT name FROM sqlite_master WHERE type='trigger';"
   ```
...
**Symptoms**: `/plugin install rad-mem` fails.
...
**Solution**: Check `~/.rad-mem/` exists and is writable.
...
2. **Create issue**: [GitHub Issues](https://github.com/thedotmack/rad-mem/issues)
...
- [Configuration](configuration) - Customize Rad-Mem
