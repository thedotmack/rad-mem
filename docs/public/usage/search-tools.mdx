...
Once rad-mem is installed as a plugin, you can search your project history using natural language. Claude automatically invokes the mem-search skill when you ask about past work.
...
**v5.4.0 Architecture**: Rad-Mem uses a skill-based search architecture instead of MCP tools, saving ~2,250 tokens per session start through progressive disclosure.
...
All search results include citations using the `rad-mem://` URI scheme:

- `rad-mem://observation/123` - Specific observation
- `rad-mem://session/abc-456` - Specific session
- `rad-mem://user-prompt/789` - Specific user prompt
