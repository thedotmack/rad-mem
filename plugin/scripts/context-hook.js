...
`+JSON.stringify(i,null,2):n=" "+this.formatData(i));let f="";if(r){let{sessionId:T,sdkSessionId:R,correlationId:l,...p}=r;Object.keys(p).length>0&&(f=` {${Object.entries(p).map(([x,A])=>`${x}=${A}`).join(", ")}}`)}let b=`[${a}] [${d}] [${m}] ${g}${t}${f}${n}`;e===3?console.error(b):console.log(b)}debug(e,s,t,r){this.log(0,e,s,t,r)}info(e,s,t,r){this.log(1,e,s,t,r)}warn(e,s,t,r){this.log(2,e,s,t,r)}error(e,s,t,r){this.log(3,e,s,t,r)}dataIn(e,s,t,r){this.info(e,`\u2192 ${s}`,t,r)}dataOut(e,s,t,r){this.info(e,`\u2190 ${s}`,t,r)}success(e,s,t,r){this.info(e,`\u2713 ${s}`,t,r)}failure(e,s,t,r){this.error(e,`\u2717 ${s}`,t,r)}timing(e,s,t,r){this.info(e,`\u23F1 ${s}`,r,{duration:`${t}ms`})}},Q=new H;var U=class{db;constructor(){J(L),this.db=new me(q),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.ensurePlatformColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).run(e,e,s,t,r.toISOString(),i);return d.lastInsertRowid===0||d.changes===0?(s&&s.trim()!==""&&this.db.prepare(`
      ...
      `).get(e).id):d.lastInsertRowid}ensureSession(e,s,t,r){let i=this.db.prepare(`
      SELECT id, prompt_counter FROM sdk_sessions WHERE claude_session_id = ?
    `).get(e);if(i){let n=(i.prompt_counter||0)+1;return r?this.db.prepare(`
          UPDATE sdk_sessions
          SET prompt_counter = ?, user_prompt = ?, project = COALESCE(NULLIF(?, ''), project)
          WHERE id = ?
        `).run(n,r,t,i.id):this.db.prepare(`
          UPDATE sdk_sessions
          SET prompt_counter = ?, project = COALESCE(NULLIF(?, ''), project)
          WHERE id = ?
        `).run(n,t,i.id),r&&this.saveUserPrompt(e,n,r),{id:i.id,prompt_number:n,created:!1}}let a=new Date,d=a.getTime(),g=this.db.prepare(`
      INSERT INTO sdk_sessions
      (claude_session_id, sdk_session_id, project, platform, user_prompt, prompt_counter, started_at, started_at_epoch, status)
      VALUES (?, ?, ?, ?, ?, 1, ?, ?, 'active')
    `).run(e,e,t,s,r||null,a.toISOString(),d).lastInsertRowid;return r&&this.saveUserPrompt(e,1,r),{id:g,prompt_number:1,created:!0}}updateSDKSessionId(e,s){return this.db.prepare(`
      UPDATE sdk_sessions
      SET sdk_session_id = ?
