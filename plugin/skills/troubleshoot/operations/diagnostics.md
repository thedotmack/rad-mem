...
Comprehensive step-by-step diagnostic workflow for rad-mem issues.
...
**Expected output:** JSON array with `rad-mem-worker` process showing `"status": "online"`
...
sqlite3 ~/.rad-mem/rad-mem.db "SELECT COUNT(*) as observation_count FROM observations;" 2>&1

# Query for session count
sqlite3 ~/.rad-mem/rad-mem.db "SELECT COUNT(*) as session_count FROM sessions;" 2>&1

# Check recent observations
sqlite3 ~/.rad-mem/rad-mem.db "SELECT created_at, type, title FROM observations ORDER BY created_at DESC LIMIT 5;" 2>&1
```
...
# Or use local pm2:
cd ~/.claude/plugins/marketplaces/thedotmack/
node_modules/.bin/pm2 logs rad-mem-worker --lines 50 --nostream

# Check for specific errors
pm2 logs rad-mem-worker --lines 100 --nostream | grep -i "error\|exception\|failed"
```
...
cat ~/.rad-mem/settings.json 2>/dev/null
...
```bash
#!/bin/bash
echo "=== Rad-Mem Troubleshooting Report ==="
echo ""
echo "1. Environment"
echo "   OS: $(uname -s)"
echo ""
...
echo ""
echo "3. Database"
echo "   Database exists: $([ -f ~/.rad-mem/rad-mem.db ] && echo 'YES' || echo 'NO')"
echo "   Database size: $(du -h ~/.rad-mem/rad-mem.db 2>/dev/null | cut -f1)"
echo "   Observation count: $(sqlite3 ~/.rad-mem/rad-mem.db 'SELECT COUNT(*) FROM observations;' 2>/dev/null || echo 'N/A')"
echo "   Session count: $(sqlite3 ~/.rad-mem/rad-mem.db 'SELECT COUNT(*) FROM sessions;' 2>/dev/null || echo 'N/A')"
echo ""
...
echo "   Latest observation: $(sqlite3 ~/.rad-mem/rad-mem.db 'SELECT created_at FROM observations ORDER BY created_at DESC LIMIT 1;' 2>/dev/null || echo 'N/A')"
echo "   Latest session: $(sqlite3 ~/.rad-mem/rad-mem.db 'SELECT created_at FROM sessions ORDER BY created_at DESC LIMIT 1;' 2>/dev/null || echo 'N/A')"
echo ""
echo "=== End Report ==="
```

Save this as `/tmp/rad-mem-diagnostics.sh` and run:
```bash
bash /tmp/rad-mem-diagnostics.sh
```
...
2. Worker logs: `pm2 logs rad-mem-worker --lines 100 --nostream`
3. Your setup:
...
Post to: https://github.com/thedotmack/rad-mem/issues
