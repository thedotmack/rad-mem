...
async function buildViewer() {
  ...
  try {
    ...
    await esbuild.build({
      ...
      target: ['es2020'],
      format: 'iife',
      outfile: path.join(rootDir, 'dist/ui/viewer-bundle.js'),
      jsx: 'automatic',
      loader: {
        '.tsx': 'tsx',
        '.ts': 'ts'
    ...
    fs.writeFileSync(
      path.join(rootDir, 'dist/ui/viewer.html'),
      htmlTemplate
    );

    // Copy font assets
    const fontsDir = path.join(rootDir, 'src/ui/viewer/assets/fonts');
    const outputFontsDir = path.join(rootDir, 'dist/ui/assets/fonts');
    ...
    const outputUiDir = path.join(rootDir, 'dist/ui');
    ...
    console.log('  - dist/ui/viewer-bundle.js');
    console.log('  - dist/ui/viewer.html (from viewer-template.html)');
    console.log('  - dist/ui/assets/fonts/* (font files)');
    console.log(`  - dist/ui/icon-thick-*.svg (${iconFiles.length} icon files)`);
  } catch (error) {
